...
A2: Biovolume decreased after timepoint 4 for each group. Also, Kelp exudate nitrate phosphate can be higher in biovolume than glucose nitrate phosphate at most of the time. The Kelp Exudate treatment's biocolume could even drop into a lower point than the control group at timepoint 8.
...
A3:The specific growth rate is calculated from log phase of the curve. According to DAPI data, we calculate this rate only based on two timepoint, which means we don't know what exactly is the point log phase ended. In that case, our result would not be so accurate.
...
A4: During the first 2 days, the changes bacterial carbon and biovolume are very similar to each other. However, the trend become slightly different after 2 days when the nutrient are almost consumed by bacterias. In the control group, bacteria carbon slope remains the same after 2 days, but biovolume goes down. For the kelp exudate group and the exudate plus nutrient group, bacterial carbon slightly increase. However, biovolume decrease. For the glucose group, both biovolume and bacteria carbon decrease, but biovolume decreases in a larger degree. A possible reason might be bacteria are consumed by flagellates, and flagellates prefer larger bacteria. This would lead to the decrease of biovolume.
...
A5: Doubling time is not line up with specific growth rate and delta cells. This is because a shorter the doubling time is, the faster the cells will grow. In that case, we would see a reverse pattern between doulbling time and the other two. For the oxygen drawdown data, if the doubling time is shorter, it means the cells grows faster. Thus, oxygen will be consumed faster.
...
A6: a) For control group, target concentration of carbon is 0 uM, for other groups, target TOC is 10 uL.
    b) We hit our target in each treatment.
    c) Since target TOC is the same for all groups except the control group, we can use the result to assess how much TOC bacterial is able to use in each treatment.
    d) TOC for all group dramatically decrease in the first 2 days for all treatments. After 2 days, they decrease in a slower rate, which means the nutrients added are running off. It is align with our data.
...
A7: For delta DOC, incorporating biovolume, the kelpa plus nutrient group slightly increase, and becomes bigger than kelp exudate group. This is closer to our expectation. Since biovolume indicate the volume of cells in a unit amount of water, biovolume-derrived delta DOC would help us to assess the data more accurately by providing a more accurate carbon conversion factor.In that case, for delta DOC, the data tell us exudate plus nutrients group utilize DOC more efficiently and consumed more DOC. For bioavailable DOC fraction, incorporating biovolume would cause similar change with delta DOC, it tell us there are more bioavailable DOC in exudate plus nutrients group. For BGE, in both graphs BGE is much higher for exudate + nutrient group than glucose group. A potential reason is flagellates population grows faster if there are more bacteria. As a result, many bacteria was consumed by flagellates in the glucose group. For the control group, the BGE is much higher in the graph without incorparating biovolume. One explanation is their biovolume is very low. In that case, if we neglect biovolme, we would overestimate their BGE.